...
import { ScheduleItemVM, ScheduleVM } from '../../model';
...
export class GetClassroomScheduleService {
  ...
  exec(schedule: ScheduleVM): Observable<Array<ScheduleItemVM>> {
    return this.http
      .get(
        `http://localhost:3000/schedules?classroomId=${schedule.classroomId}&dayId=${schedule.dayId}&_expand=classroom&_expand=day&_expand=section`
      )
